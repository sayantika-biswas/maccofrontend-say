...
//           >
//             {/* LEFT SECTION */}
//             <div className="bg-gray-100 lg:w-1/2 p-8 flex flex-col items-center justify-center text-center h-[1300px]">
...
//             {/* RIGHT SECTION (FORM) */}
//             <div className="lg:w-2/3 p-8 bg-white overflow-y-auto lg:pt-20" >
...
//     if (validate()) {
//       alert('Form submitted successfully!');
//       // TODO: Handle submission logic here, e.g., send formData and files to an API
//       console.log('Form Data:', formData);
//       console.log('Resume File:', resumeFile);
//       console.log('Additional Documents File:', additionalDocumentsFile);
//     } else {
...
export default function ApplyNow() {
  ...
  const [formData, setFormData] = useState({
    ...
    previousOrg: '',
    role: '',
    // jobDesc: '', // This field was in the form but not in the state, consider adding if needed.
    projectTitle: '',
    projectDesc: '',
    linkedIn: '',
    github: '',
  ...
  const validate = () => {
    ...
    // First Name Validation: Mandatory, first letter capital, only a-z characters
    if (!formData.firstName.trim()) {
      newErrors.firstName = 'First Name is required.';
    } else if (!/^[A-Z][a-zA-Z]*$/.test(formData.firstName)) {
      newErrors.firstName = 'First Name must start with a capital letter and contain only alphabetic characters.';
    }

    // Last Name Validation: Mandatory, first letter capital, only a-z characters
    if (!formData.lastName.trim()) {
      newErrors.lastName = 'Last Name is required.';
    } else if (!/^[A-Z][a-zA-Z]*$/.test(formData.lastName)) {
      newErrors.lastName = 'Last Name must start with a capital letter and contain only alphabetic characters.';
    }

    // Email Validation: Mandatory, includes '@' and a valid domain
    if (!formData.email.trim()) {
      newErrors.email = 'Email is required.';
    } else if (!/\S+@\S+\.\S+/.test(formData.email)) {
      newErrors.email = 'Email is invalid. It must contain "@" and a valid domain.';
    }
    ...
    // Current CTC Validation: Mandatory, numeric
    if (!formData.currentCTC.trim()) {
      newErrors.currentCTC = 'Current CTC is required.';
    } else if (isNaN(formData.currentCTC)) {
      newErrors.currentCTC = 'Current CTC must be a number.';
    }

    // Expected CTC Validation: Mandatory, numeric
    if (!formData.expectedCTC.trim()) {
      newErrors.expectedCTC = 'Expected CTC is required.';
    } else if (isNaN(formData.expectedCTC)) {
      newErrors.expectedCTC = 'Expected CTC must be a number.';
    }

    // Experience Validation: Mandatory
    if (!formData.experience) {
      newErrors.experience = 'Experience is required.';
    }

    // Previous Organization Validation: Mandatory
    if (!formData.previousOrg.trim()) {
      newErrors.previousOrg = 'Previous Organization is required.';
    }
    ...
    // Job Description Validation: Mandatory (if you add it to state)
    // if (!formData.jobDesc.trim()) {
    //   newErrors.jobDesc = 'Job Description is required.';
    // }

    // Project Title Validation: Mandatory
    if (!formData.projectTitle.trim()) {
      newErrors.projectTitle = 'Project Title is required.';
    }

    // Project Description Validation: Maximum 200 words
    const projectDescWordCount = formData.projectDesc.trim().split(/\s+/).filter(Boolean).length;
    if (!formData.projectDesc.trim()) {
      newErrors.projectDesc = 'Project Description is required.';
    } else if (projectDescWordCount > 200) {
      newErrors.projectDesc = `Project Description exceeds the 200-word limit. Current word count: ${projectDescWordCount}.`;
    }

    // LinkedIn URL Validation: Mandatory, basic URL format
    if (!formData.linkedIn.trim()) {
      newErrors.linkedIn = 'LinkedIn URL is required.';
    } else if (!/^https:\/\/[a-zA-Z0-9]+\.linkedin\.com\/.*/.test(formData.linkedIn)) {
      newErrors.linkedIn = 'Please enter a valid LinkedIn URL (e.g., https://www.linkedin.com/in/yourprofile).';
    }

    // GitHub URL Validation: Mandatory, basic URL format
    if (!formData.github.trim()) {
      newErrors.github = 'GitHub URL is required.';
    } else if (!/^https:\/\/(www\.)?github\.com\/.*/.test(formData.github)) {
      newErrors.github = 'Please enter a valid GitHub URL (e.g., https://github.com/yourprofile).';
    }

    // Resume File Validation: Mandatory
    if (!resumeFile) {
          ...
          >
            ...
            <div className="lg:w-2/3 p-8 bg-white overflow-y-auto lg:pt-17" >
              <form className="space-y-6" onSubmit={handleSubmit} noValidate>
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <div className="w-full">
                    ...
                    <input
                      ...
                      value={formData.phone}
                      onChange={handleInputChange}
                      className={`w-full mt-1 p-3 border rounded-md focus:outline-none focus:ring-1 focus:ring-rose-500 ${errors.phone ? 'border-red-500' : 'border-gray-300'}`}
                    />
                ...
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="w-full">
                    <label className="block text-sm font-semibold text-gray-700">Previous Organization<span className="text-red-500">*</span></label>
                    <input
                      name="previousOrg"
                      type="text"
                      value={formData.previousOrg}
                      onChange={handleInputChange}
                      className={`w-full mt-1 p-3 border rounded-md focus:outline-none focus:ring-1 focus:ring-rose-500 ${errors.previousOrg ? 'border-red-500' : 'border-gray-300'}`}
                    />
                    {errors.previousOrg && <p className="text-red-500 text-sm mt-1">{errors.previousOrg}</p>}
                  </div>
                  <div className="w-full">
                    <label className="block text-sm font-semibold text-gray-700">Your Role<span className="text-red-500">*</span></label>
                    <select
                ...
                {/* Project Info */}
                <div>
                  <label className="block text-sm font-semibold text-gray-700">Previous Project Title<span className="text-red-500">*</span></label>
                  <input
                    name="projectTitle"
                    type="text"
                    value={formData.projectTitle}
                    onChange={handleInputChange}
                    className={`w-full mt-1 p-3 border rounded-md focus:outline-none focus:ring-1 focus:ring-rose-500 ${errors.projectTitle ? 'border-red-500' : 'border-gray-300'}`}
                  />
                  {errors.projectTitle && <p className="text-red-500 text-sm mt-1">{errors.projectTitle}</p>}
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700">Project Description<span className="text-red-500">*</span></label>
                  <textarea
                    name="projectDesc"
                    rows={4}
                    value={formData.projectDesc}
                    onChange={handleInputChange}
                    className={`w-full mt-1 p-3 border rounded-md focus:outline-none focus:ring-1 focus:ring-rose-500 ${errors.projectDesc ? 'border-red-500' : 'border-gray-300'}`}
                  ></textarea>
                  {errors.projectDesc && <p className="text-red-500 text-sm mt-1">{errors.projectDesc}</p>}
                </div>
